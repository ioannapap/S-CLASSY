...
def multinomial_with_recurrence(cardinality, n):
    """ Data encoding: Computes the Normalized Maximum Likelihood (NML) code length
    ...
    if cardinality == 1:
        ...
        bound = int(ceil(2 + sqrt(2 * n * d * log(10))))
...
@lru_cache(maxsize=20000, typed=True)
def universal_code_integers(value: int) -> float:   # model encoding
...
def universal_code_integers_maximum(n: int, maximum: int) -> float:
    """ Model encoding code 1: computes the universal code of integers when there is a known maximum integer
    This is equivalent to applying the maximum entropy principle knowing the maximum,
    and its equalitarian division of the non-used probability (the ones after the maximum)
...
def uniform_combination_code(n: int, maximum: int) -> float:
    """ Model encoding code 2: Code based on n-combination of maximum.
