...
from rulelist.search.iterative_rule_search import greedy_and_beamsearch_PRL, greedy_and_beamsearch_HRL
from rulelist.util.bitset_operations import bitset2indexes
from rulelist.datastructure.data import Data

from rulelist.util.append_to_file import append_file
from rulelist.util.clear_file import clear_content_before_append
...
class BaseRuleList(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, Y, class_labels):
        """Fit the model according to the given training datastructure. Class labels (list format) are required only when we binarize
        Parameters
        ----------
        ...
        if is_nominal_target:
            ...
            self._rulelist = GaussianRuleList(data, self.task, self.max_depth, self.beam_width, self.min_support, self.max_rules,
                                                       self.alpha_gain)
        self._rulelist = greedy_and_beamsearch_PRL(data, self._rulelist)
        self._rulelist.add_description(class_labels)
        self.runtime = time() - start_time
        print("CLASSY runtime (sec):", str(self.runtime))
        print("\n")
        append_file("CLASSY_runtime.txt", str(self.runtime))
    ...
    def fit_S_CLASSY(self, X, Y, class_labels, expert_knowledge_filename):
        """Fit the model according to the given training datastructure, class labels (in list format) are required only when we binarize
        Parameters
        ----------
        df : pandas dataframe with name variables with last column as target
        variable.
        ...
        data = Data(input_data=X, n_cutpoints=self.n_cutpoints, discretization=self.discretization, target_data=Y, target_model=self.target_model, min_support=self.min_support)

        if is_nominal_target:
            self._rulelist = CategoricalRuleList(data, self.task, self.max_depth, self.beam_width, self.min_support,
                                             self.max_rules,
                                             self.alpha_gain)
        else:
            self._rulelist = GaussianRuleList(data, self.task, self.max_depth, self.beam_width, self.min_support,
                                          self.max_rules,
                                          self.alpha_gain)
        self._rulelist = greedy_and_beamsearch_HRL(data, self._rulelist, expert_knowledge_filename)
        self._rulelist.add_description(class_labels)
        self.runtime = time() - start_time
        if expert_knowledge_filename == "expert_knowledge_top_K.csv":
            print("S-CLASSY (top) runtime - sec: ", str(self.runtime))
            append_file("S-CLASSY_top_runtime.txt",  str(self.runtime))
        elif expert_knowledge_filename == "expert_knowledge_bottom_K.csv":
            print("S-CLASSY (bottom) runtime - sec: ", str(self.runtime))
            append_file("S-CLASSY_bottom_runtime.txt",  str(self.runtime))
        elif expert_knowledge_filename == "expert_knowledge_random_K.csv":
            print("S-CLASSY (random) runtime - sec: ", str(self.runtime))
            append_file("S-CLASSY_random_runtime.txt",  str(self.runtime))
        print("\n")
        self.number_rules = self._rulelist.number_rules
        self.rule_sets = [bitset2indexes(bitset) for bitset in self._rulelist.bitset_rules]
...
class RuleListClassifier(ClassifierMixin, BaseRuleList):
    ...
    References:
     ...
     """
    def __init__(self, *, target_model="categorical", task="prediction", max_depth=4, beam_width=100,
                 min_support=5, n_cutpoints=5, discretization="static",
                 max_rules=np.inf, alpha_gain=1.0):

        super().__init__(max_depth=4,
                         beam_width=100,
                         min_support=5,
                         n_cutpoints=5,
    ...
    clear_content_before_append("S-CLASSY_top_relative_compression.txt")
    clear_content_before_append("S-CLASSY_bottom_relative_compression.txt")
    clear_content_before_append("S-CLASSY_random_relative_compression.txt")
    clear_content_before_append("CLASSY_relative_compression.txt")
    clear_content_before_append("S-CLASSY_top_runtime.txt")
    clear_content_before_append("S-CLASSY_bottom_runtime.txt")
    clear_content_before_append("S-CLASSY_random_runtime.txt")
    clear_content_before_append("CLASSY_runtime.txt")
...
class RuleListRegressor(RegressorMixin, BaseRuleList):
    ...
    References:
     ...
     """
    def __init__(self,*,max_depth=4, beam_width = 100,
                 min_support=5, n_cutpoints = 5, discretization = "static",
                 max_rules = np.inf, alpha_gain = 1.0):
...
class SubgroupListCategorical(ClassifierMixin, BaseRuleList):
    """A probabilistic subgroup list for nominal targets.
     ...
     It can be applied for finding subgroup lists in data with univariate or multivariate (independent) targets.
...
class SubgroupListGaussian(RegressorMixin, BaseRuleList):
    """A probabilistic subgroup list for numeric targets (modelled with a normal distribution).
     ...
     Parameters
     ----------
     max_depth : int, optional (default=5) # I changed it to 4 because of paper reffering to 4.
...
def RuleList(task = "prediction", target_model = "categorical", max_depth=4, beam_width=100, min_support=5, n_cutpoints=5,
    ...
    if task == "prediction" and target_model == "categorical":
        return RuleListClassifier(max_depth=4, beam_width=100,min_support=5, n_cutpoints=5,
             discretization="static", max_rules=0, alpha_gain=1.0)
    elif task == "prediction" and target_model == "gaussian":
        return RuleListRegressor(max_depth=4, beam_width=100,min_support=5, n_cutpoints=5,
             discretization="static", max_rules=0, alpha_gain=1.0)
    elif task == "discovery" and target_model == "categorical":
        return SubgroupListCategorical(max_depth=4, beam_width=100,min_support=5, n_cutpoints=5,
             discretization="static", max_rules=0, alpha_gain=1.0)
    elif task == "discovery" and target_model == "gaussian":
        return SubgroupListGaussian(max_depth=4, beam_width=100,min_support=5, n_cutpoints=5,
             discretization="static", max_rules=0, alpha_gain=1.0)