...
def compute_delta_data(rulelist, new_subgroup_statistics, new_default_rule_statistics):
    """ Computes the alpha_gain (delta) in code length of adding one rule to the model.
...
def compute_delta_score(rulelist, new_candidate, new_subgroup_statistics, new_default_rule_statistics):
    ...
    delta_score = (delta_data+delta_model) / (usage ** rulelist.alpha_gain)     # normalised gain
...
def compute_statistics_newrules(rulelist, data, bitarray_subgroup):
    ...
    """
    rulelist.tmp_subgroup_statistic = rulelist.tmp_subgroup_statistic.replace_stats(data, bitarray_subgroup) #1. e.g. from print,       CategoricalFreeStatistic(usage=75, number_targets=1, usage_per_class={0: {4: 12, 1: 32, 6: 6, 2: 13, 7: 5, 3: 5, 5: 2}}, number_classes={0: 7})
    bitarray_new_defaultrule = rulelist.bitset_uncovered &~ bitarray_subgroup
    rulelist.tmp_default_statistic = rulelist.tmp_default_statistic.replace_stats(data, bitarray_new_defaultrule) #2. e.g. from print,  CategoricalFreeStatistic(usage=0, number_targets=1, usage_per_class={0: {4: 0, 1: 0, 6: 0, 2: 0, 7: 0, 3: 0, 5: 0}}, number_classes={0: 7})
